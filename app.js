(function () {
  ...
  async function loadSettingsFromSupabase() {
    try {
      const data = await supabaseRequest('/rest/v1/store_settings?id=eq.store&select=*');
  ...
  async function saveSettings() {
    ...
    try {
      const cfg = getSupabaseConfig();
      if (cfg) {
        await supabaseRequest('/rest/v1/store_settings?on_conflict=id', {
          method: 'POST',
          headers: {
      ...
      // ignore Supabase write issues and keep local storage fallback
    }
  ...
  async function loadProductsFromSupabase() {
    try {
      const data = await supabaseRequest('/rest/v1/products?select=*');
  ...
  async function syncProductsToSupabase(productList) {
    ...
    try {
      ...
      await supabaseRequest('/rest/v1/products?on_conflict=id', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Prefer: 'resolution=merge-duplicates'
  ...
  async function handleAddProduct(e) {
    ...
    try {
      ...
      alert('Product added. It is now synced to your database when Supabase is configured.');
    } catch (err) {
