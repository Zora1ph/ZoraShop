(function () {
      ...
      <article class="product-card" data-id="${product.id}">
        <div class="product-card-media">
          ...
          <div class="product-card-actions">
            <button class="btn btn-primary btn-full choose-btn" data-id="${product.id}">View product</button>
          </div>
        </div>
  ...
  const renderProductGallery = (images) => {
    galleryImages = (Array.isArray(images) ? images : []).filter(Boolean).slice(0, 8);
  ...
  const showStaticPayment = (order) => {
    ...
    const waitEl = $('#paymentWaitStatus');
    if (waitEl) {
      waitEl.textContent = 'Waiting for you to send payment…';
  ...
  const bindLookbookEditors = (container) => {
    ...
    container.querySelectorAll('.lookbook-file').forEach((input) => {
      input.addEventListener('change', async (e) => {
        ...
        const item = list?.[Number(input.dataset.index)];
        if (!file || !item) return;
        item.image = await compressImage(file, 2000, 0.92);
  ...
  const openEditProduct = (id) => {
    ...
    editor.addEventListener('submit', async (event) => {
      ...
      if (files.length) {
        const next = await Promise.all(files.map((file) => compressImage(file, 2000, 0.92)));
  ...
  $('#addProductForm')?.addEventListener('submit', async (e) => {
    ...
    const images = await Promise.all(files.map((file) => compressImage(file, 2000, 0.92)));
  ...
  $('#addHeroSlideBtn')?.addEventListener('click', () => {
    ...
    showToast('New hero slide added. Upload a photo, then Save lookbook.');
  });
  ...
  const startOrderWatch = () => {
    ...
    let known = JSON.parse(localStorage.getItem('zora_known_orders') || '[]');
    const check = async () => {
      try {
        const data = await supaRequest('/rest/v1/orders?select=id,name,phone,total,payment_method,created_at&order=created_at.desc');
        if (!Array.isArray(data)) return;
        const ids = data.map((o) => o.id);
        const fresh = data.filter((o) => known.length && !known.includes(o.id));
        fresh.forEach(notifyNewOrder);
        known = ids;
        localStorage.setItem('zora_known_orders', JSON.stringify(known));
      } catch {
        /* ignore */
      }
    };
