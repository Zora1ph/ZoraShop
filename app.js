(function () {
  ...
  const SIZE_PRESETS = {
    rings: ['6', '7', '8', '9'],
    earrings: ['2mm', '3mm', '4mm', '5mm', '6mm'],
    bracelets: ['17CM', '19CM', '21CM'],
    necklaces: ['40CM', '45CM', '50CM']
  };
  ...
  const bindSizePicker = (root, { category, selected } = {}) => {
    const row = root.querySelector('[data-size-presets]');
    const input = root.querySelector('[name="sizes"]');
    const select = root.querySelector('[name="category"]');
    if (!row || !input) return;

    const render = (cat, chosen) => {
      const presets = SIZE_PRESETS[cat] || SIZE_PRESETS.rings;
      const active = chosen && chosen.length ? chosen.filter((size) => presets.includes(size)) : [...presets];
      row.innerHTML = presets
        .map(
          (size) =>
            `<button type="button" class="size-preset-btn${active.includes(size) ? ' active' : ''}" data-size="${escapeHtml(size)}">${escapeHtml(size)}</button>`
        )
        .join('');
    ...
    const currentSizes = () =>
      [...row.querySelectorAll('.size-preset-btn.active')].map((btn) => btn.dataset.size);

    render(category || select?.value || 'rings', selected);

    row.onclick = (e) => {
      const btn = e.target.closest('.size-preset-btn');
      if (!btn) return;
      btn.classList.toggle('active');
      input.value = currentSizes().join(', ');
    };

    if (select && !select.dataset.sizePickerBound) {
      select.dataset.sizePickerBound = '1';
      select.addEventListener('change', () => {
        render(select.value, SIZE_PRESETS[select.value] || []);
      });
    }
  };
  ...
  const sendCustomerEmail = async ({ to, subject, name, message }) => {
    const email = String(to || '').trim().toLowerCase();
    if (!email) return false;
    let delivered = false;
    try {
      const created = await invokeEdge('send-reset-code', { email, subject, name, message });
      if (created?.ok) delivered = true;
    } catch {
      /* local fallbacks */
    }
    if (!delivered) {
      try {
        ...
        body.append('_captcha', 'false');
        body.append('_replyto', STORE_EMAIL);
        body.append('name', name || 'ZORA customer');
        ...
        const data = await res.json().catch(() => ({}));
        if (res.ok && data.success !== false && data.success !== 'false') delivered = true;
      } catch {
        /* next */
      }
    }
  ...
  const saveResetOnAccount = async (email, codeHash, expiresIso) => {
    const local = findLocalAccount(email);
    if (local) {
      local.resetCodeHash = codeHash;
      local.resetExpires = expiresIso;
      upsertLocalAccount(local);
    }
    try {
      ...
      /* columns may still be missing until SQL is run */
    }
  };
  ...
  const requestPasswordReset = async (email) => {
    ...
    const generic = 'If this email has a ZORA account, we sent a 6-digit code. Check Inbox and Spam.';
    if (!account) return generic;
    const code = String(Math.floor(100000 + Math.random() * 900000));
    const codeHash = await hashResetCode(normalized, code);
    const expiresIso = new Date(Date.now() + 15 * 60 * 1000).toISOString();
    await saveResetOnAccount(normalized, codeHash, expiresIso);
    const sent = await sendCustomerEmail({
      to: normalized,
      name: account.name || 'ZORA customer',
      subject: 'Your ZORA password reset code',
      message:
        'Hi' +
        (account.name ? ' ' + account.name : '') +
        ',\n\nUse this 6-digit code to reset your ZORA password:\n\n' +
        code +
        '\n\nThis code expires in 15 minutes. If you did not ask to reset your password, you can ignore this email.\n\nZORA'
    });
    if (!sent) {
      throw new Error('Could not send the email yet. Check Inbox/Spam and try again.');
    }
    return generic;
  };
  ...
  const compressImage = (file, maxSize = 2000, quality = 0.92) =>
  ...
  const readProductPhotosFromList = async (list, fallbackImages = []) => {
    ...
    for (const slot of slots) {
      ...
      if (file) {
        out.push(await compressImage(file, 1600, 0.86));
        continue;
      }
      const preview = slot.querySelector('.admin-photo-preview')?.getAttribute('src');
      if (preview) out.push(preview);
    }
    return out.length ? out.slice(0, MAX_PRODUCT_PHOTOS) : fallbackImages.filter(Boolean).slice(0, MAX_PRODUCT_PHOTOS);
  };
  ...
  const closeModal = (id) => {
    ...
    if (id === 'productModal') closeGalleryLite();
  ...
  const openProductModal = (id) => {
    ...
    $('#modalProductSubtitle').textContent = activeProduct.subtitle || 'Crafted for everyday wear. Minimal, lasting, and made to be yours.';
  ...
  $('#sizeChartBtn')?.addEventListener('click', () => {
    showToast('Rings 6–9 · Bracelets 17/19/21CM · Necklaces 40/45/50CM · Earrings one size');
  });
  ...
  const setPaymentWaitUi = (paid, waitingText) => {
    ...
    if (paid) {
      waitEl.textContent = 'Successfully paid';
      waitEl.classList.add('is-paid');
      waitEl.classList.remove('is-unpaid');
    } else {
      waitEl.textContent = waitingText || 'No payment has been made';
  ...
  const markOrderPaidAuto = async (order, source) => {
    ...
    order.status = 'Paid';
    order.payment_method = order.payment_method || 'QRPh';
    storeLocalOrder(order);
    setPaymentWaitUi(true);
    setFormMessage($('#paymentMessage'), 'Successfully paid', 'success');
    renderAccountDetails();
    if (already) return;
    try {
    ...
    showToast('Successfully paid — store notified');
  };
  ...
  const pollPaymongoStatus = async (order) => {
    ...
    try {
      ...
      if (!order.paymongo_intent_id || !order.paymongo_client_key) {
        setPaymentWaitUi(false, 'No payment has been made');
        return;
      }
  ...
  const showStaticPayment = (order) => {
    const method = order.payment_method || 'GCash';
    if ($('#paymentTitle')) $('#paymentTitle').textContent = 'Pay with ' + method;
    $('#paymentInstructions').textContent =
      `Open ${method} and scan this QR. Exact amount: ${formatPrice(order.total)}. After you pay, keep your receipt. We confirm the order from your name and phone.`;
    $('#paymentDetails').innerHTML = `
      ...
      <img class="payment-qr" src="${qrForMethod(method)}" alt="${escapeHtml(method)} QR" />
      <p class="payment-note">Scan with ${escapeHtml(method)}. Amount must match ${formatPrice(order.total)}.</p>`;
    const waitEl = $('#paymentWaitStatus');
    if (waitEl) {
      setPaymentWaitUi(false, 'No payment has been made');
    }
  ...
  const showPaymentModal = async (order) => {
    ...
    if ($('#paymentTitle')) $('#paymentTitle').textContent = 'Pay with QRPh';
    $('#paymentInstructions').textContent =
    ...
    try {
      ...
      $('#paymentDetails').innerHTML = `
        ...
        <p class="payment-note">Open GCash / Maya / your bank app → Scan QR → Pay. Stay here until it says Successfully paid.</p>
      `;
      setPaymentWaitUi(false, 'No payment has been made');
  ...
  $('#checkoutForm')?.addEventListener('submit', async (e) => {
    ...
    await sendStoreEmail({
      subject: `New Zora order ${order.id} — ${order.payment_method} — ${order.name} / ${order.phone}`,
      name: order.name,
      email: order.user_email,
      message: buildOrderMessage(order, 'UNPAID — waiting for ' + order.payment_method + ' payment'),
    ...
    showToast('Pay with the QR for ' + order.payment_method);
    btn.disabled = false;
    btn.textContent = 'Place Order';
  });
    ...
    box.innerHTML = orders
      .map((order) => {
        ...
        <div class="order-card">
          ...
          <p>${isPaidStatus(order.status) ? 'Successfully paid' : 'No payment has been made'} · ${escapeHtml(order.payment_method || '')}</p>
    ...
    if ($('#accountLead')) {
      $('#accountLead').textContent =
        view === 'register'
          ...
          : view === 'forgot'
            ? 'Enter the email on your ZORA account. We only email a code if that account exists.'
            : view === 'reset'
  ...
  const renderCollections = () => {
    ...
    banners.innerHTML = collections
      .map(
        (c) => `
      <a href="#shop/${c.filter || c.category || 'all'}" class="collection-banner" data-filter="${c.filter || c.category || 'all'}">
  ...
  const switchAdminTab = (tab) => {
    ...
    if (tab === 'add') bindSizePicker($('#addProductForm'), { category: $('#addProductForm')?.category?.value || 'rings' });
  };
  ...
  const bindLookbookEditors = (container) => {
    ...
    container.querySelectorAll('.lookbook-file').forEach((input) => {
      input.addEventListener('change', async (e) => {
        ...
        const item = list?.[Number(input.dataset.index)];
        if (!file || !item) return;
        const maxSize = group === 'hero' ? 1400 : 1600;
        const quality = group === 'hero' ? 0.8 : 0.88;
        try {
    ...
    list.innerHTML = rows
      .map((order) => {
        ...
        <div class="order-card" data-order-id="${escapeHtml(order.id)}">
          <span class="order-queue">QUEUE #${queue}${isNew ? ' · NEW' : ''}</span>
          <h4>${escapeHtml(order.id)}</h4>
          <p><strong>When:</strong> ${escapeHtml(order.created_at ? new Date(order.created_at).toLocaleString() : '')}</p>
          <p><strong>Payment:</strong> ${isPaidStatus(order.status) ? 'Successfully paid' : 'No payment has been made'}</p>
          ...
          <div class="payment-identity">
            ...
            <p><strong>Full name</strong><span class="copy-line">${escapeHtml(order.name || '')}</span></p>
            <p><strong>Phone / number</strong><span class="copy-line">${escapeHtml(order.phone || '')}</span></p>
            <p><strong>Full address</strong><span class="copy-line">${escapeHtml(order.address || '')}</span></p>
          ...
          <p class="admin-hint">Match GCash / Maya / bank amount to this name and phone, then Mark Paid.</p>
        </div>`;
      })
      .join('');
  ...
  const openEditProduct = (id) => {
    ...
    editor.innerHTML = `
      ...
      <div class="admin-edit-section">
        ...
        <div class="admin-size-picker">
          <span class="admin-size-label">Sizes</span>
          <div class="size-preset-row" data-size-presets></div>
          <input type="hidden" name="sizes" value="${escapeHtml((p.sizes || []).join(', '))}" />
          <p class="admin-hint">Tap every size this product offers. Earrings use 2mm–6mm.</p>
        </div>
    ...
    bindSizePicker(editor, { category: p.category, selected: p.sizes || [] });
  ...
  $('#addProductForm')?.addEventListener('submit', async (e) => {
    ...
    bindSizePicker(form, { category: form.category.value });
    renderProducts();
    switchAdminTab('products');
  ...
  const init = async () => {
    ...
    initProductZoom();
    updateCartUI();
    if ($('#addProductForm')) bindSizePicker($('#addProductForm'), { category: 'rings' });
    fillProductPhotoList($('#addProductPhotos'));
    if (!getPaymongoPublicKey()) $('#qrphOption')?.classList.add('hidden');
    startOrderWatch();
    refreshAdminEntry();
    await loadProducts();
    applyLocation();
