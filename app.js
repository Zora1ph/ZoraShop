(function () {
  ...
  const bindSizePicker = (root, { selected } = {}) => {
    ...
    const addTab = (value = '') => {
      ...
      tab.innerHTML = `
        <input type="text" data-size-tab value="${escapeHtml(value)}" placeholder="#" maxlength="16" inputmode="text" autocomplete="off" />
  ...
  const buildOrderMessage = (order, paidStatus) => {
    ...
    (order.items || []).forEach((item) => {
      ...
      lines.push(`  • ${item.name}${size}${color} × ${qty} — ${formatPrice((Number(item.price) || 0) * qty)}`);
    });
    ...
    try {
      await supaRequest(`/rest/v1/orders?id=eq.${encodeURIComponent(order.id)}`, {
        ...
        body: JSON.stringify({
          ...
          notes: `promo=${order.promo || ''};paymongo=paid;source=${source || 'poll'};method=${order.payment_method || ''}`
        })
      });
    } catch {
      /* ignore */
    ...
    box.innerHTML = orders
      .map((order) => {
        ...
        <div class="order-card">
          ...
          <p>${isPaidStatus(order.status) ? 'Paid' : 'Waiting for payment'} · ${escapeHtml(paymentLabel(order.payment_method))}</p>
