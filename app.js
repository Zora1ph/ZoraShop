(function () {
  ...
  // Local paths to the QR images you uploaded. These use file:/// URLs so they load when
  // you open index.html locally. Replace with hosted URLs when deploying to a remote server.
  const QR_PATHS = {
    'GCash': 'file:///C:/Users/rewar/AppData/Roaming/Code/agentSessionData/b58b1897-1ec6-427e-83db-0b6e2ea92fbc/attachments/60682b15-b9a6-4901-b857-a6db3eb23129/Pasted%20Image.png',
    'Bank Transfer': 'file:///C:/Users/rewar/AppData/Roaming/Code/agentSessionData/b58b1897-1ec6-427e-83db-0b6e2ea92fbc/attachments/23bb9fd0-8523-48b3-b66d-17008d085c22/Pasted%20Image%202.png',
    'Maya': 'file:///C:/Users/rewar/AppData/Roaming/Code/agentSessionData/b58b1897-1ec6-427e-83db-0b6e2ea92fbc/attachments/1c2b0c9c-0b60-4319-b8a0-29583ed6d634/Pasted%20Image%203.png'
  };
  ...
  function saveCart() {
    const key = currentAccount ? `zora_cart_${currentAccount.email}` : STORAGE_KEYS.cart;
  ...
  function loadCart() {
    ...
    if (currentAccount) {
      const guestCart = JSON.parse(localStorage.getItem(STORAGE_KEYS.cart) || '[]');
      if (saved.length === 0 && Array.isArray(guestCart) && guestCart.length) {
        cart = guestCart;
        saveCart();
      } else {
        cart = saved;
  ...
  function getSavedAccount() {
    try {
      return JSON.parse(localStorage.getItem('zora_account_user') || 'null');
    } catch {
      return null;
    }
  }
  ...
  function saveAccount() {
    localStorage.setItem('zora_account_user', JSON.stringify(currentAccount));
  }

  function getAccounts() {
    try {
      return JSON.parse(localStorage.getItem('zora_accounts') || '[]');
    } catch {
      return [];
    }
  }
  ...
  function saveAccounts(accounts) {
    localStorage.setItem('zora_accounts', JSON.stringify(accounts));
  }
  ...
  function openCheckoutChannel(channel, message, winRef) {
    // Messenger/Instagram-based checkout removed. This function is a no-op placeholder
    // until an in-site payment flow is added. For now, orders are created and the UI
    // shows an in-site toast to proceed to payment.
    return;
  }
  ...
  function openProductDetail(id) {
    ...
    const detailGalleryEl = $('#detail-gallery');
    if (detailGalleryEl) {
      detailGalleryEl.classList.remove('zoomed');
    }
    if (mainImg) {
      ...
      mainImg.onclick = () => {
        if (detailGalleryEl) {
          detailGalleryEl.classList.toggle('zoomed');
        }
      };
    }
  ...
  function openAccountModal(mode = 'login') {
    if (mode === 'account' && currentAccount) {
      ...
      $('#account-login-form').classList.remove('hidden');
      $('#account-logged-in').classList.add('hidden');
      $('#account-logged-out').classList.remove('hidden');
    }
    ...
    document.body.style.overflow = 'hidden';
    if (mode === 'account') renderAccountDetails();
  }
  ...
  function renderAccountDetails() {
    ...
    $('#account-welcome').textContent = `Logged in as ${currentAccount.email}`;
    $('#account-email').textContent = currentAccount.email;
    $('#account-order-count').textContent = `${orders.length} order${orders.length === 1 ? '' : 's'}`;
    const accountOrders = $('#account-orders');
    if (!accountOrders) return;
    if (!orders.length) {
      accountOrders.innerHTML = '<p class="cart-empty">No orders have been placed yet.</p>';
      return;
    }
    accountOrders.innerHTML = orders.map(order => {
      ...
      <div class="order-card">
        <h3>Order ${order.id || '(Pending)'}</h3>
        <p><strong>Status:</strong> <span class="status-badge">${order.status || 'Pending'}</span></p>
        <p><strong>Payment:</strong> ${order.payment_method || 'Unknown'}</p>
        <p><strong>Total:</strong> ${formatPrice(order.total || 0)}</p>
        <p><strong>Date:</strong> ${new Date(order.created_at || order.createdAt || Date.now()).toLocaleString()}</p>
        <p><strong>Items:</strong></p>
        <ul>${items.map(item => `<li>${item.name}${item.size ? ` (${item.size})` : ''} × ${item.qty}</li>`).join('')}</ul>
      </div>`;
    }).join('');
  }
  ...
  async function loadOrdersForEmail(email) {
    orders = [];
    if (!email) return;
    const storedOrders = localStorage.getItem(`zora_orders_${email.toLowerCase()}`);
    if (storedOrders) {
      try {
    ...
    if (cfg) {
      try {
        const rows = await supaRequest(`/rest/v1/orders?user_email=eq.${encodeURIComponent(email.toLowerCase())}&order=created_at.desc`);
        if (Array.isArray(rows)) {
          const existingIds = new Set(rows.map(row => row.id));
          const localMap = new Map(orders.map(order => [order.id, order]));
          const merged = rows.map(row => localMap.get(row.id) || row);
          const localOnly = orders.filter(order => !existingIds.has(order.id));
          orders = merged.concat(localOnly);
        }
      } catch {
        // ignore supabase read failures
      }
    }
  ...
  function storeLocalOrder(order) {
    ...
    let existingOrders = [];
    try {
      existingOrders = JSON.parse(localStorage.getItem(`zora_orders_${email}`) || '[]');
    } catch {
      existingOrders = [];
    }
    existingOrders.unshift(order);
    localStorage.setItem(`zora_orders_${email}`, JSON.stringify(existingOrders));
  ...
  function saveSupportMessage(message) {
    ...
    supportMessages.unshift(payload);
    try {
      localStorage.setItem('zora_support_messages', JSON.stringify(supportMessages));
    } catch {
      // ignore local storage failure
    }
    const cfg = getSupabaseConfig();
    if (cfg) {
      supaRequest('/rest/v1/support_messages', {
        method: 'POST',
  ...
  function getPaymentInstructions(method) {
    switch (method) {
      case 'GCash':
        return 'Scan the GCash QR below and send the total amount. Use your email as reference.';
      case 'Maya':
        return 'Scan the Maya QR below and send the total amount. Use your email as reference.';
      case 'Bank Transfer':
        return 'Scan the bank QR below or use the account info. Use your email as reference and send the confirmation to zoraofficial005@gmail.com.';
      default:
        return 'Follow the instructions provided to complete the payment.';
    }
  }

  function showPaymentModal(order) {
    const instructions = getPaymentInstructions(order.payment_method);
    $('#payment-instructions').textContent = instructions;
    const details = $('#payment-details');
    if (details) {
      const qrSrc = QR_PATHS[order.payment_method] || '';
      const wrap = details.querySelector('#payment-qr-wrap');
      if (wrap) {
        wrap.innerHTML = `
          <div class="order-info" style="text-align:left;margin-bottom:8px;color:var(--white)">
            <p><strong>Order #:</strong> ${order.id}</p>
            <p><strong>Total:</strong> ${formatPrice(order.total)}</p>
            <p><strong>Payment:</strong> ${order.payment_method}</p>
          </div>
          <img id="payment-qr" src="${qrSrc}" alt="${order.payment_method} QR" style="max-width:320px;border:1px solid rgba(255,255,255,0.12);padding:6px;background:#fff;">
          <p style="margin-top:8px;color:var(--grey);font-size:0.9rem">Send payment and include your email as reference. For questions, email <strong>zoraofficial005@gmail.com</strong></p>
        `;
      }
    }
  ...
  async function handlePaymentConfirm() {
    if (!lastOrder) {
      alert('No order found to confirm.');
      closePaymentModal();
      return;
    }

    // Mark the order as Processing locally and attempt to update Supabase (best-effort)
    lastOrder.status = 'Processing';
    storeLocalOrder(lastOrder);

    // Try to patch the order status in Supabase if configured
    try {
      const cfg = getSupabaseConfig();
      if (cfg) {
        await supaRequest(`/rest/v1/orders?id=eq.${encodeURIComponent(lastOrder.id)}`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
    ...
    closePaymentModal();
    alert('Payment received. Your order is now being processed. You can track it from your account.');
    const toast = $('#order-toast');
  ...
  function buildOrderMessage(formData) {
    ...
    lines.push(`PAYMENT: ${formData.paymentMethod || 'N/A'}`);
    lines.push('');
  ...
  async function handleCheckout(e) {
    ...
    const form = e.target;
    const paymentMethod = form.paymentMethod?.value || 'GCash';
    if (!paymentMethod) {
    ...
    if (!formData.name || !formData.phone || !formData.address) {
      alert('Please complete your contact details before placing the order.');
      return;
    }

    const order = {
      id: generateId(),
      user_email: currentAccount?.email?.toLowerCase() || null,
      items: cart.map(item => ({ id: item.id, name: item.name, size: item.size, qty: item.qty, price: item.price })),
      total: getCartTotal(),
    ...
    // Open mail client pre-filled to send order details to store owner (zoraofficial005@gmail.com).
    // This lets the customer's email client send the order to the store's Gmail inbox.
    try {
      const mailto = 'mailto:zoraofficial005@gmail.com?subject=' + encodeURIComponent('New order ' + order.id) + '&body=' + encodeURIComponent(message);
      // open in new window/tab so we don't navigate away in the shop
      window.open(mailto, '_blank');
    } catch (err) {
      // no-op if mailto fails
    }
  ...
  function openEditProduct(id) {
    ...
    editor.innerHTML = `
      ...
      <label class="admin-field">
        ...
        <input name="subtitle" value="${escapeHtml(p.subtitle || '')}" placeholder="Example: SILVER">
  ...
  async function init() {
    ...
    try {
      supportMessages = JSON.parse(localStorage.getItem('zora_support_messages') || '[]') || [];
    } catch {
      supportMessages = [];
    }
    if (currentAccount) {
    ...
    $('#help-btn')?.addEventListener('click', openSupportModal);
    $('#payment-confirm-btn')?.addEventListener('click', handlePaymentConfirm);
    $('#goto-payment')?.addEventListener('click', () => {
      if (lastOrder) showPaymentModal(lastOrder);
    });

    // Allow customers to continue shopping after placing an order
    $('#continue-shopping-btn')?.addEventListener('click', () => {
      const toast = $('#order-toast');
      if (toast) toast.classList.add('hidden');
      // scroll to top/main shopping area
      document.querySelector('main')?.scrollIntoView({ behavior: 'smooth' });
    });

    // Close payment modal and continue shopping
    $('#payment-close-continue')?.addEventListener('click', () => {
      closePaymentModal();
      const toast = $('#order-toast');
      if (toast) toast.classList.add('hidden');
      document.querySelector('main')?.scrollIntoView({ behavior: 'smooth' });
    ...
    $('#account-register-form')?.addEventListener('submit', async (e) => {
      ...
      currentAccount = findAccount(email);
      saveAccount();
      loadCart();
      await loadOrdersForEmail(currentAccount.email);
      openAccountModal('account');
    ...
    $('#show-register')?.addEventListener('click', () => openAccountModal('register'));
    $('#show-login')?.addEventListener('click', () => openAccountModal('login'));
    ...
    $('#close-support')?.addEventListener('click', closeSupportModal);
