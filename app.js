(function () {
  ...
  const loadProducts = async () => {
    try {
      ...
      if (Array.isArray(data) && data.length) {
        ...
        try {
          localStorage.setItem('zora_products_cache', JSON.stringify(products.map((p) => ({ ...p, images: (p.images || []).slice(0, 2) }))));
        } catch {
          /* quota */
        }
        renderProducts();
  ...
  const compressImage = (file, maxSize = 1100, quality = 0.74) =>
  ...
  const readProductPhotosFromList = async (list, fallbackImages = []) => {
    ...
    for (const slot of slots) {
      ...
      if (file) {
        out.push(await compressImage(file, 1100, 0.74));
        continue;
      }
  ...
  const bindLookbookEditors = (container) => {
    ...
    container.querySelectorAll('.lookbook-file').forEach((input) => {
      input.addEventListener('change', async (e) => {
        ...
        const item = list?.[Number(input.dataset.index)];
        if (!file || !item) return;
        const maxSize = group === 'hero' ? 1100 : 1200;
        const quality = group === 'hero' ? 0.74 : 0.76;
        try {
  ...
  $('#addProductForm')?.addEventListener('submit', async (e) => {
    ...
    const saved = await saveProductRemote(products[0]);
    form.reset();
