    ...
    try {
      if (cfg) {
        ...
        try {
          ...
          if (!String(err.message || '').includes('hero_slides')) throw err;
          const { hero_slides, ...legacyPayload } = settingsPayload;
          await supaRequest('/rest/v1/store_settings?on_conflict=id', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
          ...
          lastSupabaseError = 'Product and account settings saved. Run the hero_slides migration to publish hero edits.';
        }
