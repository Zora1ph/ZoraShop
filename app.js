(function () {
  ...
  async function loadSettingsFromSupabase() {
    try {
      ...
      return {
        ...
        heroSlides: Array.isArray(row.hero_slides) ? row.hero_slides : DEFAULT_SETTINGS.heroSlides
      };
    } catch {
      return null;
    }
  ...
  async function saveDraft() {
    try {
      ...
      alert(
        ...
        'Tips:\n• Use smaller / fewer photos\n• Click SYNC TO DATABASE then clear draft\n\n' +
  ...
  function updatePublishBanner() {
    ...
    if (hasUnpublishedChanges) {
      ...
      banner.innerHTML =
        ...
        '<em>SYNC TO DATABASE</em> and the Supabase tables are updated.';
    } else {
  ...
  async function publishProducts() {
    ...
    if (syncedToSupabase) {
      ...
      alert(
        'Supabase sync was skipped because the config is not set.\n\n' +
        'products.json + store-settings.json downloaded instead.'
      );
    }
  }
  ...
  function getMessengerUrl(message) {
    const encoded = encodeURIComponent(message);
    // Prefer an explicit facebookUrl if provided (try to extract a usable page identifier)
    ...
    if (fbUrl && fbUrl.includes('facebook.com')) {
      try {
        ...
        if (candidate) return `https://m.me/${candidate}?text=${encoded}`;
      } catch (e) {
        // fallthrough
      }
    }
    ...
    if (page) return `https://m.me/${page}?text=${encoded}`;

    // Last resort: use a generic fallback
    return `https://m.me/Zora.Official.ph?text=${encoded}`;
  }
  ...
  function openEditProduct(id) {
    ...
    let sizes = parseSizesInput(newSizes);
    if (sizes.length > stock) {
      sizes = validateSizesForStock(sizes, stock);
    }
  ...
  async function init() {
    ...
    // Render hero slides from settings then initialize carousel
    renderHeroSlides();
    initHeroCarousel();
    ...
    // Prepare hero slides admin editor (hidden until admin opens panel)
    renderHeroSlidesEditor();
