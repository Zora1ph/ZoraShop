(function () {
  ...
  const loadSettingsMedia = async () => {
    try {
      const data = await supaRequest(
        '/rest/v1/store_settings?id=eq.store&select=hero_slides,collections,category_cards,blogs'
      );
      const row = Array.isArray(data) && data[0];
      if (!row) return;
      if (Array.isArray(row.hero_slides) && row.hero_slides.length) settings.heroSlides = row.hero_slides;
  ...
  const loadProducts = async () => {
    try {
      const data = await supaRequest(`/rest/v1/products?select=${PRODUCT_LIST_COLS}&order=created_at.desc`);
  ...
  const switchAdminTab = (tab) => {
    ...
    if (tab === 'payments') {
      loadPaymentQrs().then(() => renderPaymentPreviews());
    }
  ...
  const bindLookbookEditors = (container) => {
    ...
    container.querySelectorAll('.lookbook-remove').forEach((btn) => {
      btn.addEventListener('click', async () => {
        ...
        const saved = await patchStoreSettings(lookbookPatchFields(btn.dataset.group));
        refreshStorefront();
  ...
  const renderAdminOrders = async () => {
      ...
      .map((order) => {
        ...
        <div class="order-card${paid ? ' order-card--paid' : ''}" data-order-id="${escapeHtml(order.id)}">
          ...
          <div class="admin-ship">
            <p class="admin-hint">Courier tracking (customer sees this in My account and on the product)</p>
            ...
            <select data-courier><option value="">Courier</option>${COURIERS.map((c) => `<option${c === getShipment(order).courier ? ' selected' : ''}>${escapeHtml(c)}</option>`).join('')}</select>
            <input data-track-no placeholder="Waybill / tracking number" value="${escapeHtml(getShipment(order).trackingNo)}" />
            <input data-track-url placeholder="Tracking link (optional)" value="${escapeHtml(getShipment(order).trackingUrl)}" />
            <button type="button" class="btn btn-primary" data-save-ship data-id="${escapeHtml(order.id)}">Update tracking</button>
          </div>
        </div>`;
      })
      .join('');
    ...
    list.querySelectorAll('[data-save-ship]').forEach((btn) => {
      btn.addEventListener('click', () => {
        const card = btn.closest('.order-card');
        saveOrderShipment(btn.dataset.id, {
          ship: card?.querySelector('[data-ship-status]')?.value || 'paid',
          courier: card?.querySelector('[data-courier]')?.value || '',
          trackingNo: card?.querySelector('[data-track-no]')?.value.trim() || '',
          trackingUrl: card?.querySelector('[data-track-url]')?.value.trim() || ''
        });
      });
    });
  };
